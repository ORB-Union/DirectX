...
bool SuperBullet::check_collision(float x, float y)
{
...
void SuperBullet::init(float x, float y)
{
	x_pos = x;
	y_pos = y;
...
void SuperBullet::move()
{
	y_pos -= 20;
}

void SuperBullet::hide()
{
	bShow = false;
}
...
void do_game_logic(void)
{
	...
	if(enemybullet.show() == true)
	{
		if (enemybullet.y_pos > 500)
			enemybullet.hide();
		else
			enemybullet.move();
	}
