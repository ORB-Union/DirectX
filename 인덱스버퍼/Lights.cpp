...
{
	int colorindex = 0;
	// 메모장에서 데이터(인덱스버퍼)를 읽어들이기
	int cindex = 0;
	FILE *fl = fopen("custom.txt", "r");
	...
	FILE *f2 = fopen("color.txt", "r");

	CUSTOMVERTEX custom;
	CUSTOMVERTEX *cu; 
	g_pVB->Lock(0, sizeof(CUSTOMVERTEX), (void**)&cu, 0);
	while (fscanf(fl, "%f %f %f", &custom.x, &custom.y, &custom.z) != EOF)
	{
		if ((cindex) % 1 == 0)
		{
			fscanf(f2, "%id", &custom.color);
		...
		if ((cindex)%2 == 0)
		{
		custom.color = colorbuffer[colorindex];
		colorindex++;
		}
		*/
		cindex++;
...
HRESULT InitIB()
{
	...
	*/
	...
	g_pIB->Lock(0, sizeof(MYINDEX), (void**)&my, 0);
	while (fscanf(f3, "%hd %hd %hd", &myindex._0, &myindex._1, &myindex._2) != EOF)
	{
		my[index] = myindex;
		index++;
	}
